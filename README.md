...
- **趋势上下文**：简报中每个标的显示与 30 分钟前的量比趋势对比（↑↓→）
...
信号触发时推送富文本卡片，包含价格、量比、LLM 分析。多个信号合并为单张表格卡片，涨跌用 🔴🟢⚪ 色标区分方向。简报标题自动区分"有关注"和"无异常"，每个标的附带与 30 分钟前的量比趋势对比（↑↓→）。
...
| v3.8 | 2026-05-07 | 全盘代码审查修复 27 项（SQLite 错误处理/BaseException/缓存上限/竞态/fd 泄漏/PID 生命周期等）；飞书消息 10 项产品优化（简报标题"有关注"/LLM 分析提权/价格符号按市场/趋势上下文/状态卡片层级/信号方向标识/触发来源标注/静默到期通知/LLM prompt 加日内量比/告警合并为单张卡片）；尾盘信号检测扩展至港股和美股 |
