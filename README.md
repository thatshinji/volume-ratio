...
- **JSONL + DuckDB 存储**：JSONL 保存可回放行情快照，DuckDB 保存分钟聚合、量比结果、原始快照索引和信号历史（含胜率回填）
...
┌────────────────────────▼────────────────────────────────────┐
│                    数据层                                    │
│     snapshots/*.jsonl        │       ratios.duckdb         │
│  (JSONL行情快照，按天追加)     │ (DuckDB分钟聚合+量比+信号历史) │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│                    数据源层                                   │
...
│   └── ratios.duckdb        # DuckDB 数据库（量比+快照索引+信号历史）
│
└── logs/                    # 运行日志
```
